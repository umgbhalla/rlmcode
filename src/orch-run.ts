...
// whole stack end-to-end: the core primitives (leaf/parallel/emit/allocate) and the
// recipes built on them (agent/judge/adversarialVerify/loopUntilDry) actually fan
...
//     ├─ candidate 1   ┐  parallel() fan-out — each leaf forwards over a FORKED
//     ├─ candidate 2   ┘  AxMemory (never the session mem, never shared between
//     ├─ judge            concurrent leaves), so two branches can't corrupt each
...
//                         only on this boundary fiber, never inside a leaf.
//
...
// leaves answer straight from the prompt.
 ...
 * tally the UI surfaces. Individual leaf failures degrade gracefully — parallel()
...
export const orchestrate = (parent: AnySpan, sessionId: string, message: string) =>
  Effect.fn("chat.orchestrate", {
    ...
    // leaves never mutate a shared history. The session's real mem is owned by turn().
...
const run = async (
  ...
  try {
    ...
    // fan-out of agent() nodes, each over its own forked memory.
    const survivors = await loopUntilDry(
      () => fanOut(rootId, message, optsFor, budget),
      (prev, next) => prev.length === next.length,
      2,
    )
    if (survivors.length === 0) throw new Error("all candidate leaves failed")

    // 2) JUDGE: one leaf picks the best candidate verbatim. Bracketed so a throw
    ...
    // their own lifecycle events via agent()).
...
// One parallel() fan-out of candidate agent() nodes. Each branch pre-emits its own
// parent edge (agent()'s start carries no parentId), runs the leaf over a forked
...
const fanOut = async (
  ...
  const raw = await parallel(
    candidateGens.map((gen, i) => () => {
      ...
      // agent()'s own (parentId-less) start lands second; atoms preserves parentId.
      onEvent({ type: "start", nodeId, parentId: rootId, phase: `candidate ${i + 1}` })
      return node(nodeId, gen, optsFor(), budget, { message }).then((o) => (o as { reply?: string }).reply ?? "")
    }),
  )
  return raw.filter((r): r is string => typeof r === "string" && r.length > 0)
}

// A single skeptic leaf voting on the answer. Bracketed as its own node so the tree
...
const skeptic = async (
  ...
  const out = await node(nodeId, skepticGen, opts, budget, { message, answer })
  return /accept/i.test(String((out as { verdict?: string }).verdict ?? ""))
}

// Thin wrapper over the agent() recipe: same nodeId so its done/error updates the
// edge node in place, with budget charging wired (usageOf reads the leaf's usage).
// ponytail: the two candidate / two skeptic leaves share ONE AxGen instance each and
...
// over/under-charge by up to one leaf's usage under contention. Upgrade: construct a
// fresh AxGen per branch (one instance per concurrent forward) so getUsage() is 1:1.
const node = <I extends AxGenIn, O extends AxGenOut>(
  nodeId: string,
  gen: AxGen<I, O>,
  opts: LeafOpts,
  budget: Budget,
  ...
  agent({ nodeId, gen, opts, onEvent, budget, usageOf: (g) => readUsageOf(g) }, llm, input)
