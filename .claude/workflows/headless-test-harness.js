export const meta = {
  ...
  description: 'RE-GROUNDED past d014b5d (+40 commits): the test suite GREW to ~10 files (scripts/{orch-core,orch,orch-cost,orch-journal,orch-live,orch-retry,gepa,focus-live}.test.ts) — judge + loopUntilDry are NOW covered by orch-live.test.ts, so they are NO LONGER the gap. Scout MUST re-glob ALL scripts/*.test.ts and re-derive what is genuinely uncovered (do NOT assume judge/loopUntilDry/emit). The durable missing layer this build adds: (a) a reusable no-LLM collector/driver (scripts/headless-harness.ts) factoring the inline helpers duplicated across the existing test files, and (b) a runTurn() TurnEvent-stream golden/snapshot test (scripts/turn-stream.test.ts) — NOT yet buildable, runTurn ships in core-tui-split, so land it as a skip-with-reason stub. Any recipe-gap test is ONLY for whatever the scout finds still uncovered. Matches ax2 assert-fixture no-framework style, wires into `bun run test`, green under `bun run lint`. Sequential on main, self-heal + adversarial review per deliverable, commit each.',
  phases: [
    ...
    { title: 'Scout',      detail: 'parallel read-only: re-glob ALL scripts/*.test.ts (~10 now: orch-core/orch/orch-cost/orch-journal/orch-live/orch-retry/gepa/focus-live) and pin what each ALREADY covers — judge+loopUntilDry are covered by orch-live.test.ts, so re-derive the REAL remaining gap (do NOT assume judge/loopUntilDry/emit); pin the runTurn/TurnEvent contract if present, and the assert-fixture template + test-runner wiring' },
    ...
    { title: 'recipe-gaps', detail: 'ONLY whatever the Scout found genuinely uncovered across all ~10 test files (judge/loopUntilDry are already covered by orch-live.test.ts — do NOT re-add them). If the scout finds NO real gap, SKIP this phase — the durable value is the shared harness + the runTurn golden stub, not redundant recipe tests.' },
...
// the actual source at ax2 (commit ~d014b5d). Do NOT contradict
// it; do NOT re-do what the two existing orch tests already cover.
// ---------------------------------------------------------------------------
const CORE_SPEC = `
ax2 headless test harness. The codebase under test lives at ax2.
