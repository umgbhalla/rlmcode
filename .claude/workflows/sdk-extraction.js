export const meta = {
  ...
  phases: [
    ...
    { title: 'createAgent-di', detail: 'DI: move llm/chat/answerGen singleton construction into createAgent({ai,tools,maxSteps,model,tokenBudget}) -> { turn, abortTurn }; turn() closes over the injected config instead of module globals; keep module-level defaultAgent + re-exported turn/abortTurn so atoms.ts is byte-identical; thread injected ai into orch-run.ts AND orch-load.ts (both import the bare llm today)' },
