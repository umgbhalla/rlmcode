export const meta = {
  name: 'orch-tree-ui',
  description: 'TWO jobs: (1) FIX THE RENDER, (2) re-route the event path. The current chat.tsx NodeView renders the orch tree as DEPTH-INDENTED rows with a status glyph and NO tree connectors — it reads as a flat indented blob, not a tree. The store reduce + orchestrate() wiring exist and are fine; the RENDER is the real defect. Fix: replace the nested-box + paddingLeft NodeView with a proper velocity UNICODE TREE — flatten the OrchTree (roots + childrenIndex) into ordered rows, prefix each row with a connector string built from ancestor last-child flags (├─ for a non-last child, └─ for the last, │ continuation, blank past a resolved ancestor), column-align the status glyph + label + one-line summary + token badge, and keep the live running-auto-expand + collapse toggle + the Σ run-total footer. THEN (2) once core-tui-split ships runTurn(): add a TurnEvent kind:\'node\' variant, push orch.emit node events into the in-flight per-turn buffer instead of the global setActivitySink, fold them in the PURE store reduce (port the parentId-preserving logic), retire the global sink path. Gate on a headless reduce snapshot test (out-of-order child resolve) AND a render-string golden test (the flattened connector lines). Sequential on main, self-heal to bun-run-check green + 2-lens adversarial review, commit each step. The RENDER fix (job 1) does NOT depend on core-tui-split and can ship first; job 2 needs runTurn.',
  phases: [
    { title: 'Scout',        detail: 'parallel read-only: pin the CURRENT NodeView render (nested box + paddingLeft + nodeGlyph, NO connectors — the defect) and the OrchTree shape (nodes/roots/parentId, childrenIndex, status/phase/result/tokens, totalTokens); pin the existing reduce + orchestrate() wiring; and check whether core-tui-split/runTurn has landed (job 2 gate; job 1 does not need it)' },
    { title: 'tree-render',  detail: 'JOB 1 (render fix, no core-tui-split dep): a PURE flatten(orch): Row[] that walks roots+children in order, computing each row\'s connector prefix from ancestor last-child flags (├─ / └─ / │ / blank); a column-aligned line (prefix + glyph + label, padded, then summary + token badge); rebuild NodeView to render the flat rows (one <text> each) instead of nested paddingLeft boxes; keep running-auto-expand, collapse toggle, hover, and the Σ totalTokens footer. Add a render-string golden test (assert the exact connector lines for a known tree incl a parallel fan-out + a nested child + an error node).' },
    { title: 'node-event',   detail: 'JOB 2 (needs runTurn): add TurnEvent kind:\'node\', push orch.emit node events into the in-flight per-turn buffer instead of global setActivitySink, fold into the pure reduce (port parentId-preserving logic), retire the global sink path. STOP if core-tui-split has not landed — ship job 1 alone.' },
    { title: 'headless-test', detail: 'orch-tree reduce snapshot (out-of-order child resolve) + the job-1 render-string golden, both plain-assert no-TUI no-LLM, wired into bun run test, exit 1 on fail.' },
    { title: 'Report',       detail: 'final status, per-step commit, before/after render sample, what flows through the TurnEvent stream vs the retired sink, residual risk, next' },
  ],
}
...
const TEST_SCHEMA = {
  ...
  properties: {
    ...
    testFile: { type: 'string', description: 'path under ax2' },
