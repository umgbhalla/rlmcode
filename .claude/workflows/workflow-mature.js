...
PATTERNS (teach these as runnable example scripts — the FIRST is the production workhorse that carries ~170-agent swarms; budget loops are secondary):
  // SURVEY-LOOP (THE workhorse — MAX_ROUNDS + zero-exit + frontier-cap, NOT budget):
  const MAX_ROUNDS=(args&&args.max_rounds)||12, CAP=16;
  for(let round=1;round<=MAX_ROUNDS;round++){ phase('Survey');
    const s=await agent('build/scan -> {total, by_file}', {sig:'_->total:number, by_file:json', label:'survey:'+round});
    if(s.total===0) return {done:true, rounds:round};
    const frontier=Object.entries(s.by_file).sort((a,b)=>(seen[a[0]]||0)-(seen[b[0]]||0)||b[1]-a[1]).slice(0,CAP); // UNSEEN first, then count desc
    for(const[f]of frontier) seen[f]=(seen[f]||0)+1;
    phase('Fix'); await pipeline(frontier, ([f,n])=>agent('fix '+n+' in '+f), (fix,[f])=>agent('verify '+f), (v,[f])=>agent('apply '+f)); }
  // SHARD (modulo, restart-safe): const SHARD=A.shard||0, NSHARDS=A.nshards||1; const mine=units.filter((_,i)=>i%NSHARDS===SHARD);
  // 2-VOTE verify (production uses 2, not 3): const v=(await parallel([0,1].map(i=>()=>agent('verify: '+claim,{sig:'_->ok:boolean, conf:string', label:'vote:'+i})))).filter(Boolean); const accept=v.length>=2 && v.every(x=>x.ok) && v.every(x=>['high','medium'].includes(x.conf));
  // fan-out + judge: phase('fan'); const c=await parallel([()=>agent('A'),()=>agent('B'),()=>agent('C')]); return await judge(c.filter(Boolean));
  // typed extract: const e=await agent('extract', {sig:'text:string -> name:string, age:number, emails:string[]'}); // e.age is a real number.
  // rlm node (mine a HUGE blob out-of-prompt): return await rlm(HUGE_LOG, 'which line throws the 500?');
  // loop-until-budget (ax2 EXTRA, secondary): const found=[]; while(budget.total && budget.remaining()>50000){ const r=await agent('find one more',{sig:'_->item:string, done:boolean'}); if(r.done)break; found.push(r.item); }
  // single-mutation isolation: parallel WRITE nodes COLLIDE in one tree — either ONE node does git/build (rest edit-only) OR use isolation:'worktree' (patch-return, rebase-not-reset).
CAPS (production-grounded): concurrency ~16; per-round frontier CAP 16; fan-out clamp 100; MAX_ROUNDS default 12; verify = 2-vote.
...
REALITY: production scripts use ONLY agent/parallel/pipeline/phase/log/args — judge/rlm/budget/per-node model are ax2 EXTRAS. The MAX_ROUNDS survey-loop + modulo-shard + 2-vote are what actually scale.`
...
const FEATURES = [
  ...
  { key: 'budget-loops', live: true, spec: `Support the PRODUCTION loop patterns (grounded in zonko/bun, 53 scripts): PRIMARY = the survey-loop — for(round=1;round<=MAX_ROUNDS;round++){ survey -> if(total===0) return done; frontier = prioritize(unseen-first).slice(0,CAP); pipeline(fix,verify,apply) } with MAX_ROUNDS default 12 + zero-survey early-exit + per-round frontier CAP (16) + a seen-set; this is THE workhorse, NOT budget. SECONDARY (ax2 extra) = budget{total,spent,remaining} for loop-until-budget; loop-until-dry (K=2 empty rounds); loop-until-count. Advisory soft never discards a completed node; HARD stops. Set caps: ORCH_CONCURRENCY ~16 (from 8), fan-out clamp 100, verify default 2-vote. LIVE: a Kimi-authored script running a real survey-loop that iterates >=2 rounds, prioritizes unseen, and early-exits on zero — paste the round-by-round output; ALSO a budget-loop that accumulates + stops on the line. tsc+lint green. commit.` },
