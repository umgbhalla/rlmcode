export const meta = {
  ...
  phases: [
    ...
    { title: 'folder-move',     detail: 'mechanical move of the FULL CURRENT headless set (re-glob src/ — it grew +9 since d014b5d): src/{agent,runtime,models,rlm-tool,orch,orch-recipes,orch-run,orch-load,orch-tools,orch-journal,orch-resilience,orch-plan,orch-tasks,orch-optimize,activity,sessions,tools,toolui}.ts -> src/core/; src/{atoms,chat}.tsx + clipboard/history -> src/tui/. NOTE runtime.ts now owns the AxAIService (the old agent.ts llm singleton moved there) and already broke the agent<->orch cycle. Fix imports only.' },
