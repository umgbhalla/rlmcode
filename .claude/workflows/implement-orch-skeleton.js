...
const baselineThunk = () =>
  agent(`Run \`${CHECK_CMD}\` then \`${LINT_CMD}\` at the repo root WITHOUT editing anything. Report whether each is currently green and list any PRE-EXISTING errors verbatim (so a later diff is not blamed for them). Read-only except running the commands.`,
...
while (impl && impl.status !== 'green' && round < MAX_FIX_ROUNDS && (!budget.total || budget.remaining() > 60000)) {
  ...
  impl = await agent(
    `The orch-skeleton edits are applied in the working tree but \`${CHECK_CMD}\`/\`${LINT_CMD}\` are RED. Diagnose and FIX in the live tree, then re-run both. Do NOT revert the skeleton; fix the cause. Behavior must stay identical to pre-refactor.\n\nFAILING OUTPUT:\ncheck: ${impl.checkOutput}\nlint: ${impl.lintOutput}\n\nPRE-EXISTING (ignore): ${JSON.stringify(baseline && baseline.preexistingErrors || [])}\n\nReturn the same structured result (diff, outputs, status, files, fixRoundsUsed, notes).\n\nSPEC:\n${SPEC}`,
...
while (impl && openBlockers.length > 0 && hardenRound < MAX_HARDEN_ROUNDS && (!budget.total || budget.remaining() > 60000)) {
  ...
  impl = await agent(
    `Review found BLOCKERS in the applied orch-skeleton. Fix each in the live working tree, preserving the 5-primitive core and behavior-identical refactor, then re-run \`${CHECK_CMD}\` and \`${LINT_CMD}\`.\n\nBLOCKERS:\n${JSON.stringify(openBlockers, null, 1)}\n\nReturn the updated structured result.\n\nSPEC:\n${SPEC}`,
  ...
  const reReview = (await parallel(REVIEW_LENSES.map(l => () =>
    agent(`Re-review the now-fixed orch-skeleton for your lens ONLY. Confirm prior blockers are resolved and no new ones introduced.\n\nLENS — ${l.focus}\n\nDIFF:\n${impl ? impl.diff : ''}\n\nSPEC:\n${SPEC}`,
